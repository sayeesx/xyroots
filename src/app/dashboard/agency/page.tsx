...
export default function AgencyDashboard() {
  ...
  useEffect(() => {
    if (!loading && (!isAuthenticated || role !== 'agency')) {
      router.push('/');
    }
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      <h1 className="font-editorial text-2xl sm:text-3xl text-white">
                        {profile?.full_name || "Agency Name"}
                      </h1>
                      <span title="Verified Agency"><FaShieldHalved className="w-5 h-5 text-xyroots-yellow" /></span>
                    </div>
                    <p className="text-xs sm:text-sm text-gray-400">
                      Consultancy / Agency • {profile?.email}
                    </p>
                  </div>
                </div>
            ...
            <div className="flex overflow-x-auto gap-2 border-b border-xyroots-border pb-px mb-8 scrollbar-none">
              {[
                ...
                { id: "vacancies", label: `Managed Vacancies (${dbJobs.length})`, icon: FaRegFileLines },
                { id: "pipeline", label: "Agency Pipeline", icon: FaCalendar },
                { id: "settings", label: "Agency Settings", icon: FaShieldHalved },
              ].map((tab) => (
                <button
                  key={tab.id}
